...
int32_t PS::get_ip_address(char *ip_address)
{
    PS::Sce::LibcInternal::Initialize();
    PS::Sce::LibSceNetCtl::Initialize();

    int ret;
    SceNetCtlInfo info;
    PS::Sce::LibcInternal::Memset_s(&info, sizeof(SceNetCtlInfo), 0, sizeof(SceNetCtlInfo));

    ret = PS::Sce::LibSceNetCtl::NetCtlInit();
    if (ret >= 0)
    {
        ret = PS::Sce::LibSceNetCtl::NetCtlGetInfo(SCE_NET_CTL_INFO_IP_ADDRESS, &info);
        if (ret >= 0)
        {
            PS::Sce::LibcInternal::Memcpy(ip_address, info.ip_address, sizeof(info.ip_address));
            PS::Sce::LibSceNetCtl::NetCtlTerm();
            return ret;
        }
    }
    return -1;
