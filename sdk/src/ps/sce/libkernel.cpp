...
uint64_t PS::Sce::Kernel::pGetCpuTemperature = 0;
...
int32_t PS::Sce::Kernel::GetCpumodeGame(int *cpumodegame)
{
    return (int32_t)PS::Breakout::call(LIBKERNEL(LIB_KERNEL_SCE_KERNEL_GET_CPUMODE_GAME), PVAR_TO_NATIVE(cpumodegame));
}
...
int32_t PS::Sce::Kernel::Sysctlbyname(const char * name, void * oldp, size_t * oldlenp, void * newp, size_t newlen)
{
    return (int32_t)PS::Breakout::call(LIBKERNEL(LIB_KERNEL_SYSCTLBYNAME), PVAR_TO_NATIVE(name), PVAR_TO_NATIVE(oldp), PVAR_TO_NATIVE(oldlenp), PVAR_TO_NATIVE(newp), newlen);
}
